		...
		while (!bQuit && (line = inReader.readLine()) != null) {
			...
			try {
				...
				switch (command.toUpperCase()) {
					...
					//usage "IN" no arguments 
					//displays information about the currently loaded graph (num of vertices and edges)
					case "IN":
					...
					// GR {number of infected} {prob of infection} {prob of recovery} {c | r} (clustered or random)
					case "GR":
						if (tokens.length == 5) {
					...
					// DVT - delete randomly generated (but valid) vertices from the graph
					// DVT {number of vertices to delete}
					case "DVT":
						if (tokens.length == 2) {
							int inputSize = Integer.parseInt(tokens[1]);
					...
					//DET to delete provided number of random edges from the graph and time the process	
					// DET {number of edges to delete}	
					case "DET":
						if (tokens.length == 2) {
							int inputSize = Integer.parseInt(tokens[1]);
